...
namespace Common.Player
{
    ...
    public class PlayerAttack : MonoBehaviour
    {
        ...
        private static PlayerAnimation PlayerAnimation => 
            global::Object.Player.Instance ? Object.Player.Animation : null;
        ...
        private Vector2 GetConeCenterDirection()
        {
            ...
            bool facingRight = PlayerAnimation ? PlayerAnimation.FacingRight :
                !global::Object.Player.Instance || !Object.Player.Animation || Object.Player.Animation.FacingRight;
            if (!facingRight) dir2 = new Vector2(-dir2.x, dir2.y);
            return dir2;
        }

        private static List<Collider2D> CastConeRays(Vector2 origin, Vector2 coneDir, float halfAngleDeg, float range,
            LayerMask enemyMask, LayerMask obstacleMask, float stepDeg)
        {
            var found = new HashSet<Collider2D>();
            int combined = enemyMask | obstacleMask; // correction
            int steps = Mathf.Max(1, Mathf.CeilToInt((halfAngleDeg * 2f) / Mathf.Max(0.1f, stepDeg)));
        ...
        {
            float elapsed = 0f;
            float kbSpeed = knockbackDistance / knockbackDuration;
        ...
        private void DrawConeGizmos()
        {
            ...
            int combined = enemyLayer | obstacleLayer;
            int steps = Mathf.Max(1, Mathf.CeilToInt((half * 2f) / Mathf.Max(0.1f, rayStepDeg)));
            const float eps = 0.01f;
            for (int i = 0; i <= steps; i++)
            {
